...
from funcoes.b_analise_carteira.bc_remuneracoes.bcb_rem_acoes.bcbb_fx_exib_df_rem_mensais_acoes import _exibir_df_rem_mensais_acoes
...
def render_aba3b_remuneracoes_acoes(df_ext_pm_apos_compra, df_ext_remuneracoes, df_rem_mensais_acoes, df_posicao_acoes):
    st.markdown(f'#### {ICONE_ACOES} {TITULO_ACOES}', text_alignment='center')
    st.caption(f'*{ICONE_ANALISE_CARTEIRA} {TITULO_ANALISE_CARTEIRA} > {ICONE_REMUNERACOES} {TITULO_REMUNERACOES} > {ICONE_ACOES} {TITULO_ACOES}*',
                text_alignment='center')


    _exibir_df_rem_mensais_acoes(df_rem_mensais_acoes=df_rem_mensais_acoes)

    # ------------------------------------------------------------------------------------------------------------- Exibindo gráficos do df_posicao
    with st.expander(f'{TITULO_GRAFICOS} *{TITULO_REMUNERACOES} > {TITULO_ACOES}*', expanded=False, icon=f'{ICONE_GRAFICOS}'):
        ...
        with st.container(border=True):
            ...
            st.plotly_chart(grafico_rem_mensais_total_acoes_por_setor, use_container_width=True)


        # ------------------------------------------------------------------------------------------------------------- Gráfico 2 
        # Reaproveitando fxs dos indicadores e gráfico do VG
        with st.container(border=True):
            # Criando df_ext_pm_apos_compra_acoes para entregar apenas ações para a fx de YonC.
            df_ext_pm_apos_compra_acoes = df_ext_pm_apos_compra.loc[df_ext_pm_apos_compra['Tipo de Ativo'] == 'Ação'].copy()
            # Criando df_ext_remuneracoes_acoes para entregar apenas ações para a fx de YonC.
            df_ext_remuneracoes_acoes = df_ext_remuneracoes.loc[df_ext_remuneracoes['Tipo de Ativo'] == 'Ação'].copy()
            # Reaproveitando fx auxiliar criada no arq de gráficos VG.
            df_rem_mensais_yonc_carteira_acoes = _criar_df_rem_mensais_yonc_carteira(df_ext_pm_apos_compra=df_ext_pm_apos_compra_acoes,
                                                                                     df_ext_remuneracoes=df_ext_remuneracoes_acoes)
            ...
            with col2:
                media_mensal_ult_12_meses_df_rem_mensais_yonc_carteira_acoes = _calc_media_mensal_ult_12_meses_df_rem_mensais_yonc_carteira(
                    df_rem_mensais_yonc_carteira=df_rem_mensais_yonc_carteira_acoes)
                st.metric('Média Mensal (últ. 12 meses)',
                          f'R$ {media_mensal_ult_12_meses_df_rem_mensais_yonc_carteira_acoes:,.2f}%', border=True)

            with col3:
                soma_mensal_ult_12_meses_df_rem_mensais_yonc_carteira_acoes = _calc_soma_mensal_ult_12_meses_df_rem_mensais_yonc_carteira(
                    df_rem_mensais_yonc_carteira=df_rem_mensais_yonc_carteira_acoes)
                st.metric('Soma (últ. 12 meses)',
                          f'R$ {soma_mensal_ult_12_meses_df_rem_mensais_yonc_carteira_acoes:,.2f}%', border=True)

            grafico_rem_mensais_yonc_carteira_acoes = _criar_grafico_rem_mensais_yonc_carteira(
                df_rem_mensais_yonc_carteira=df_rem_mensais_yonc_carteira_acoes, df_rem_mensais=df_rem_mensais_acoes)
            st.plotly_chart(grafico_rem_mensais_yonc_carteira_acoes, use_container_width=True)
