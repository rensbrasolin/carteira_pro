...
def _exibir_df_rem_mensais_fiis(df_rem_mensais_fiis):

    df_rem_mensais_fiis_exib = df_rem_mensais_fiis.copy()

    # Excluindo cols desnecessárias na hora certa.
    # Aqui em FIIs tbm excluo os tipos pois os 3 tipos de fundos serão considerados como FII
    df_rem_mensais_fiis_exib = df_rem_mensais_fiis_exib.drop(
        columns=[('', 'Tipo')])
    ...
    with st.expander(f'{TITULO_DADOS} *{TITULO_REMUNERACOES} > {TITULO_FIIS}*', expanded=False, icon=f'{ICONE_DADOS}'):
        st.dataframe(
            df_rem_mensais_fiis_exib,
