...
# -------------------------------------------------------------------------------------------------------------------- Gráfico 1a
def _criar_grafico_cm_pa_rem_df_posicao_fiis(df_posicao_fiis):

    df = df_posicao_fiis.copy()
...
# -------------------------------------------------------------------------------------------------------------------- Gráfico 1b
def _criar_grafico_cm_pa_rem_total_df_posicao_fiis(
    indicador_custo_medio_df_posicao_fiis,
    indicador_patrimonio_atual_df_posicao_fiis,
    indicador_remuneracoes_df_posicao_fiis,
    indicador_variacao_percentual_df_posicao_fiis
):
    fig = go.Figure()
    ...
    fig.add_trace(go.Bar(
        x=['TOTAL'],
        y=[indicador_custo_medio_df_posicao_fiis],
        name='CM',
        offsetgroup=1,
        marker={"color": "#ef476f"},
        showlegend=False
    ...
    fig.add_trace(go.Bar(
        x=['TOTAL'],
        y=[indicador_patrimonio_atual_df_posicao_fiis],
        name='PA',
        offsetgroup=2,
        marker={"color": "#26547c"},
        showlegend=False,
        text=[f'{indicador_variacao_percentual_df_posicao_fiis:.0f}%'],
        textposition='inside',
        ...
        textfont={
            "family": "Segoe UI",
            "size": 9,
            "color": "#ffb3c1" if indicador_variacao_percentual_df_posicao_fiis < 0 else "#b7e4c7"
        }
    ))
    ...
    fig.add_trace(go.Bar(
        x=['TOTAL'],
        y=[indicador_remuneracoes_df_posicao_fiis],
        name='Rem',
        base=[indicador_patrimonio_atual_df_posicao_fiis],
        offsetgroup=2,
        marker={"color": "#ffd166"},
        visible='legendonly'
    ))
...
# -------------------------------------------------------------------------------------------------------------------- Gráfico 2
def _criar_grafico_distrib_cm_tipo_df_posicao_fiis(df_posicao_fiis):
    ...
    fig = px.sunburst(
        df,
        path=['Tipo de Fundo', 'Ticker'],  # Hierarquia: primeiro o Tipo, depois o Ativo
        values='Custo Médio',
        color='Tipo de Fundo',  # Cores por Tipo (você pode personalizar mais abaixo)
    ...
    fig.update_layout(
        ...
        title={
            'text': 'Composição do Custo Médio por Tipo de Fundo',
            'x': 0.5,
            'y': 0.95,
            'xanchor': 'center',
            'yanchor': 'top',
...
# -------------------------------------------------------------------------------------------------------------------- Gráfico 3
def _criar_grafico_distrib_pa_tipo_df_posicao_fiis(df_posicao_fiis):
    # Ordenar valores decrescentes para consistência visual
    df = df_posicao_fiis.copy()
    ...
    fig.update_layout(
        ...
        title={
            'text': 'Composição do Patrimônio Atual por Tipo de Fundo',
            'x': 0.5,
            'y': 0.95,
            'xanchor': 'center',
            'yanchor': 'top',
...
# -------------------------------------------------------------------------------------------------------------------- Gráfico 4
def _criar_grafico_distrib_cm_segmento_df_posicao_fiis(df_posicao_fiis):
    ...
    fig = px.sunburst(
        df,
        path=['Segmento', 'Ticker'],  # Hierarquia: primeiro o Tipo, depois o Ativo
        values='Custo Médio',
        color='Segmento',  # Cores por Tipo (você pode personalizar mais abaixo)
        color_discrete_sequence=["#033270","#1368aa","#4091c9","#9dcee2","#fedfd4","#f29479","#f26a4f","#ef3c2d","#cb1b16","#65010c"]
    )
    ...
    fig.update_layout(
        ...
        title={
            'text': 'Composição do Custo Médio por Segmento',
            'x': 0.5,
            'y': 0.95,
            'xanchor': 'center',
            'yanchor': 'top',
...
def _criar_grafico_distrib_pa_segmento_df_posicao_fiis(df_posicao_fiis):
    ...
    fig.update_layout(
        ...
        title={
            'text': 'Composição do Patrimônio Atual por Segmento',
            'x': 0.5,
            'y': 0.95,
            'xanchor': 'center',
            'yanchor': 'top',
