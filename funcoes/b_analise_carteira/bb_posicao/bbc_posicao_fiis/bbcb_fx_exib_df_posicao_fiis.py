...
from icones import TITULO_DADOS, TITULO_POSICAO, TITULO_FIIS, ICONE_DADOS


# --------------------------------------------------------------------------------------------------- Fxs Indicadores
# Aqui nos indicadores do df_posicao_fiis vou reaproveitar a maioria das fxs do df_posicao,
#  mudando apenas o df que passo para elas.

def _calc_indicador_qtd_segmentos_df_posicao_fiis(df_posicao_fiis):

    # Excluindo posições zeradas do df
    df_posicao_fiis_sem_pos_liquidada = df_posicao_fiis[df_posicao_fiis['Qtd'] != 0]

    indicador_qtd_segmentos_df_posicao_fiis = df_posicao_fiis_sem_pos_liquidada['Segmento'].nunique()

    return indicador_qtd_segmentos_df_posicao_fiis
...
def _exibir_df_posicao_fiis(df_posicao_fiis):

    df_posicao_fiis_exib = df_posicao_fiis.copy()
    ...
    lista_ordem_cols = ['Ticker', 'Tipo', 'Qtd', 'Preço Médio', 'Custo Médio', 'Cotação', 'Patrimônio Atual',
                         ...
                         'Tipo de Fundo', 'Segmento',
                         ]  

    with st.expander(f'{TITULO_DADOS} *{TITULO_POSICAO} > {TITULO_FIIS}*', expanded=False, icon=f'{ICONE_DADOS}'):
        st.dataframe(
            df_posicao_fiis_exib,
