...
# -------------------------------------------------------------------------------------------------------------------- Gráfico 1a
def _criar_grafico_cm_pa_rem_df_posicao_etfs(df_posicao_etfs):

    df = df_posicao_etfs.copy()
...
# -------------------------------------------------------------------------------------------------------------------- Gráfico 1b
def _criar_grafico_cm_pa_rem_total_df_posicao_etfs(
    indicador_custo_medio_df_posicao_etfs,
    indicador_patrimonio_atual_df_posicao_etfs,
    indicador_remuneracoes_df_posicao_etfs,
    indicador_variacao_percentual_df_posicao_etfs
):
    fig = go.Figure()
    ...
    fig.add_trace(go.Bar(
        x=['TOTAL'],
        y=[indicador_custo_medio_df_posicao_etfs],
        name='CM',
        offsetgroup=1,
        marker={"color": "#ef476f"},
        showlegend=False
    ...
    fig.add_trace(go.Bar(
        x=['TOTAL'],
        y=[indicador_patrimonio_atual_df_posicao_etfs],
        name='PA',
        offsetgroup=2,
        marker={"color": "#26547c"},
        showlegend=False,
        text=[f'{indicador_variacao_percentual_df_posicao_etfs:.0f}%'],
        textposition='inside',
        ...
        textfont={
            "family": "Segoe UI",
            "size": 9,
            "color": "#ffb3c1" if indicador_variacao_percentual_df_posicao_etfs < 0 else "#b7e4c7"
        }
    ))
    ...
    fig.add_trace(go.Bar(
        x=['TOTAL'],
        y=[indicador_remuneracoes_df_posicao_etfs],
        name='Rem',
        base=[indicador_patrimonio_atual_df_posicao_etfs],
        offsetgroup=2,
        marker={"color": "#ffd166"},
        visible='legendonly'
    ))
